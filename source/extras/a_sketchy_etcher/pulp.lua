...
-- ADDED BY DREW FOR SHAKE TO ERASE FUNCTIONALITY
local gfx <const> = playdate.graphics
--> Initialize sound effects
local shake_noise = playdate.sound.sampleplayer.new('extras/a_sketchy_etcher/sounds/etcher_shake')
...
import "extras/a_sketchy_etcher/pulp-audio.lua"
...
function a_sketchy_etcher.cranked(change, acceleratedChange)
...
function a_sketchy_etcher.crankDocked()
    local script = pulp:getPlayerScript()
    if script then
...
function a_sketchy_etcher.crankUndocked()
    local script = pulp:getPlayerScript()
    if script then
...
function shake()
    ...
    --[[]
    local pulp_scale = playdate.display.getScale()
    local canvas = gfx.image.new( 400 / pulp_scale, 240 / pulp_scale )
    
    gfx.lockFocus(canvas)
        etcher_image:drawFaded(0, 0, 1 - shake_counter/ENOUGH_SHAKES, gfx.image.kDitherTypeBayer2x2)
    gfx.unlockFocus()
    etcher_screen:setImage(canvas)
    ]]

    -- if the player shakes enough to erase the image:
    if shake_counter >= ENOUGH_SHAKES then
        -- reset a-sketchy-etcher screen
        pulp:getScript("player").clear() 
        shake_counter = 0
    end
...
function pulp:start()
    ...
    pulp.player.x = last_x or pulp.startx
    pulp.player.y = last_y or pulp.starty
